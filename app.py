...
def connect_google_sheet():
    ...
    # Create a mutable copy of the credentials_info from secrets
...
class GreenKivuGasService:
    ...
    def check_for_alerts() -> List[Tank]:
...
def save_lead(name, phone, email, industry, notes=""):
    csv_path = "/content/drive/MyDrive/GASMETH_Chatbot/online_inquiry.csv"
    ...
    try:
        ...
        if not os.path.exists(csv_path):
            ...
            new_lead.to_csv(csv_path, index=False)
        else:
            new_lead.to_csv(csv_path, mode='a', header=False, index=False)
        ...
        st.error(
            f"Failed to save request to CSV: {str(e)}"
        )
