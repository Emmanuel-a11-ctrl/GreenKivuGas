...
SERVICE_ACCOUNT_JSON = "/content/greenkivu_sa.json"   # Path to uploaded JSON

# -------------------------------
# CSV fallback path (Google Drive)
# -------------------------------
CSV_FALLBACK_PATH = "/content/drive/MyDrive/GASMETH_Chatbot/online_inquiry.csv"

# Ensure directory exists for CSV fallback
os.makedirs(os.path.dirname(CSV_FALLBACK_PATH), exist_ok=True)
...
def save_lead_to_csv(name, phone, email, industry, notes):
    """Fallback: write to CSV in Google Drive."""
    try:
        data = {
            "timestamp": datetime.datetime.now().isoformat(),
        ...
        if os.path.exists(CSV_FALLBACK_PATH):
            df_existing = pd.read_csv(CSV_FALLBACK_PATH)
            df_combined = pd.concat([df_existing, df_new], ignore_index=True)
        else:
            df_combined = df_new
        df_combined.to_csv(CSV_FALLBACK_PATH, index=False)
        return True, "Saved to CSV fallback"
    except Exception as e:
        return False, str(e)
...
def save_lead(name, phone, email, industry, notes=""):
    ...
    if success:
        st.success("Thank you! Your request has been saved to our lead sheet (Google Sheets).")
    else:
        # Fallback to CSV
        st.warning(f"Google Sheets failed: {msg}. Saving to CSV fallback instead.")
        success2, msg2 = save_lead_to_csv(name, phone, email, industry, notes)
        if success2:
            st.success("Thank you! Your request has been saved to CSV fallback.")
        else:
